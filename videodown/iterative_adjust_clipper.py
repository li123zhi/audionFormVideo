...
class IterativeAdjustClipper:
    ...
    def process(self) -> Dict:
        ...
        try:
            ...
            min_count = min(len(self.current_subs), len(self.new_subs))

            for i in tqdm(range(min_count), desc="对比字幕"):
                curr_sub = self.current_subs[i]
                new_sub = self.new_subs[i]

                # 当前字幕时间（已经调整过的）
                curr_start = curr_sub.start.ordinal / 1000.0
                curr_end = curr_sub.end.ordinal / 1000.0
                ...
                # 计算差值：当前开始时间 - 新开始时间
                time_diff = curr_start - new_start

                print(f"\n对比{i+1}:")
                print(f"  当前字幕{i+1}: {curr_start:.3f}s - {curr_end:.3f}s")
                print(f"  新字幕{i+1}: {new_start:.3f}s - {new_end:.3f}s")
                ...
                if time_diff > 0:
                    ...
                    # 在 curr_start 节点前剪掉 time_diff 秒
                    # 即剪掉 [curr_start - time_diff, curr_start]
                    ...
                    if new_video:
                        ...
                        # 更新后续字幕时间戳
                        self.update_subtitle_times(i, time_diff)
                    ...
                    # 在 curr_start 节点前增加 duration_to_add 秒
                    ...
                    if new_video:
                        ...
                        # 更新后续字幕时间戳（负偏移）
                        self.update_subtitle_times(i, -duration_to_add)
                        ...
                        adjustment_log.append({
                            ...
                            'new_video': new_video,
                            'extend_point': f"{curr_start:.3f}s"
                        })
                    else:
                        print(f"  ⚠️  增加失败")
                        adjustment_log.append({
            ...
            print(f"总调整时长: {total_adjustment_time:+.3f}秒")
            print(f"\n输出视频: {final_video}")
